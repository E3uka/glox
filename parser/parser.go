...
func (p *parser) peek() token.TokenType { return p.tokens.Tokens[p.current] }
...
func (p *parser) trace_nd(nd_ident string, t token.TokenType) {
	if p.trace { fmt.Printf("%v: cur: %v, next: %v\n", nd_ident, p.tokens.Lit[p.current], p.peek()) }
}
